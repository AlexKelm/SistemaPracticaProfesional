...
async function abrirModal() {
  ...
  const clientesResp = await fetch("http://localhost:3000/clientes");
...
document.getElementById("formNuevaOrden").addEventListener("submit", async function(e) {
  ...
  try {
    const url = modoEdicion.activo ? `http://localhost:3000/ordenes/${modoEdicion.id}` : "http://localhost:3000/ordenes";
...
async function eliminarOrden(id) {
  ...
  if (!confirmar) return;
  try {
    await fetch(`http://localhost:3000/ordenes/${id}`, { method: "DELETE" });
    cargarOrdenes();
  } catch (err) {
